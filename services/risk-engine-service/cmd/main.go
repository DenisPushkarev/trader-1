...
import (
	...
	"github.com/trader-1/trader-1/services/risk-engine-service/internal/application"
	"github.com/trader-1/trader-1/services/risk-engine-service/internal/infrastructure/messaging"
...
func main() {
	logger := logging.NewLogger("risk-engine-service", getEnv("LOG_LEVEL", "info"))
	...
	handler := application.NewRiskHandler(logger, pub, rc, ruleSet)
	...
	logger.Info().Msg("risk-engine-service ready")

	go func() {
		cfg := natsclient.ConsumerConfig{
			Stream: contractsv1.StreamSignalsGenerated, Durable: "risk-engine-signals-generated",
			Subject: contractsv1.SubjectSignalsGenerated, MaxDeliver: 5,
		}
