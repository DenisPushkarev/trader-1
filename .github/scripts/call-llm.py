#!/usr/bin/env python3
"""
call-llm.py — Sends a prompt to OpenRouter and prints the response to stdout.
...
Arguments:
  --role     Agent role: architect | developer | reviewer | router
  --system   Path to system-prompt file (optional)
  --prompt   Path to user-prompt file, or '-' to read from stdin (default: -)
...
Example:
  cat prompt.txt | python3 .github/scripts/call-llm.py --role architect
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Agent role (determines model from config)",
    )
    parser.add_argument(
        "--config",
        default=".github/agent-config.yml",
    ...
    parser.add_argument(
        "--prompt",
        default="-",
        help="Path to user-prompt file, or '-' for stdin",
    )
